...
std::array<int, 500*500> map;
std::array<int, 500*500> bigger_map;
int w, h;
...
int main() {
    //bool input_is_larger_version = true;
	//std::ifstream input("example_part2.txt");
    // expected result: 315

    //bool input_is_larger_version = false;
	//std::ifstream input("example.txt");
    // expected result: 315

    bool input_is_larger_version = false;
	std::ifstream input("input.txt");
    // note: takes some time (27 minutes on my machine)
    ...
    if (!input_is_larger_version) {
        // generate complete map by copying and incrementing
        for (int ym = 0; ym < 5; ym++) {
            for (int xm = 0; xm < 5; xm++) {
                int increment = xm + ym;
                for (int y = 0; y < w; y++) {
                    for (int x = 0; x < w; x++) {
                        int new_value = map[y * w + x] + increment;
                        if ((new_value) > 9) new_value -= 9;
                        bigger_map[(h * ym + y) * w * 5 + (w * xm + x)] = new_value;
                    }                
                }
            }
        }
