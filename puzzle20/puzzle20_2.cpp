...
int main() {
	...
	for (int y = 0; y < h; y++) {
		for (int x = 0; x < w; x++) {
			xm = x - (3 * enhance_times);
			ym = y - (3 * enhance_times);
			if ((xm < 0) || (xm >= w - (2 * 3 * enhance_times))) {
				map[y * w + x] = 0;
			} else if ((ym < 0) || (ym >= h - (2 * 3 * enhance_times))) {
				map[y * w + x] = 0;
			} else {
