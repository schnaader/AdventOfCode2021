...
int main() {
	...
	*/
	...
	std::string current_number = numbers[0];
	for (auto number : numbers | std::views::drop(1)) {
		current_number = reduce_number(add_numbers(current_number, number));
	}

	int idx = 0;
	printf("magnitude of final sum: %i\n", magnitude(current_number, idx));
